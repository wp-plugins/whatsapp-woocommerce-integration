...
class WhatsAppEvent
{
   ...
   function fireCodeRequestFailedTooRecent(
    ...
    function fireCodeRequestFailedToken(
         $phone,
         $method,
         $status,
         $reason
     ) {
         $callbackEvent = function(WhatsAppEventListener $listener) use ($phone, $method, $status, $reason){
             $listener->onCodeRequestFailedToken($phone, $method, $status, $reason);
         };
         $this->fireCallback($callbackEvent);
     }
