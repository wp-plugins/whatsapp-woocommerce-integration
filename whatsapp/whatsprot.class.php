...
class WhatsProt
{
    ...
    const PICTURES_FOLDER = 'images';                     // The relative folder to store picture files
    ...
    public function codeRequest($method = 'sms', $carrier = null, $countryCode = null, $langCode = null)
    {
        ...
        if ($response->status == 'ok') {
            ...
            if (isset($response->reason) && $response->reason == "too_recent") {
        	...
        	}elseif(isset($response->reason) && $response->status == "fail"  && $response->reason == "bad_token") {
                $this->eventManager()->fire("onCodeRequestFailedToken",
                    array(
                        $this->phoneNumber,
                        $method,
                        $response->status,
       	 	...
       	 	}elseif(isset($response->reason) && $response->status == "fail"  && $response->reason == "no_routes") {
                $this->eventManager()->fire("onCodeRequestFailedRoutes",
                    array(
                        $this->phoneNumber,
                        $method,
                        $response->status,
    ...
    public function connect()
    {
        ...
        if ($Socket !== false) {
            stream_set_timeout($Socket, static::TIMEOUT_SEC, 0);
            $this->socket = $Socket;
  ...
  */
	public function sendPing()
  {
    ...
    */
     ...
     */
    protected function getMediaFile($filepath, $maxsizebytes = 20048576)
    {
        if (filter_var($filepath, FILTER_VALIDATE_URL) !== false) {
            ...
            if ($this->mediaFileInfo['filesize'] < $maxsizebytes) {
                ...
                $this->mediaFileInfo['filepath'] = static::MEDIA_FOLDER;
...
//            $height = $duplicate->getAttribute("height");
            ...
            //upload new file
			$json = false;
			$messageNode['filePath'] = $_SESSION['image_url'];
			if($this->getMediaFile($_SESSION['image_url'], (20 * (1024 * 1024)))){
    ...
    protected function sendCheckAndSendMedia($filepath, $maxSize, $to, $type, $allowedExtensions, $storeURLmedia, $caption = "")
    {
        if ($this->getMediaFile($filepath, $maxSize) == true) {
            if (in_array($this->mediaFileInfo['fileextension'], $allowedExtensions)) {
                ...
                //request upload
                $msgId = $this->sendRequestFileUpload($b64hash, $type, $this->mediaFileInfo['filesize'], $this->mediaFileInfo['filepath'], $to, $caption);
